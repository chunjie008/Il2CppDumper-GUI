...
- **Kilo AI Agent 支持** — 项目内嵌 `.kilo/skills/il2cpp-ghidra/SKILL.md`，Kilo Agent 自动加载后能完成整套 Il2CppDumper ↔ Ghidra 集成工作流
...
本工程包含 Kilo AI Agent 的 skill 定义，位于 `.kilo/skills/il2cpp-ghidra/SKILL.md`。加载后 AI Agent 可自动执行：

1. Il2CppDumper CLI dump 二进制 + metadata
2. 将 dump 结果导入 Ghidra 并运行脚本
3. 解析 `dump_ai.json` 处理函数命名和结构体恢复
4. 处理手动模式、内存 dump 等高级场景

在 Kilo TUI 或 IDE 中运行 `skill il2cpp-ghidra` 即可加载。
