...
- **AI 友好输出** — 生成结构化 JSON (`dump_ai.json`) + SQLite (`dump_ai.db`)，专为 AI/LLM 解析优化
- **Il2CppDumper ↔ Ghidra 集成 Skill** — `docs/skill.md` 涵盖完整逆向工作流
...
Il2CppDumper GUI：https://github.com/chunjie008/Il2CppDumper-GUI/releases
...
| `--no-ai` | 跳过 AI dump 输出（等效于 `--ai-format none`） |
...
# 只生成 AI dump（JSON + SQLite）+ dummy dll
Il2CppDumper.exe libil2cpp.so global-metadata.dat output/ --no-dump-cs --no-struct --scripts

# 只生成 SQLite AI dump
Il2CppDumper.exe libil2cpp.so global-metadata.dat output/ --ai-format sqlite

# 只生成 JSON AI dump
Il2CppDumper.exe libil2cpp.so global-metadata.dat output/ --ai-format json
```
...
- 嵌套类型索引
- 字符串字面量
- **内置 `prompt` 字段** — 告知 AI 文件结构和解析方式，降低使用门槛

> 键名说明：`n`=名称, `v`=可见性, `t`=类型, `a`=VA地址, `ra`=RVA, `o`=偏移, `s`=签名, `p`=父类, `tk`=Token, `sz`=大小, `ab`=抽象, `sl`=密封, `vt`=值类型, `en`=枚举, `if`=接口, `st`=静态, `vr`=虚方法

#### dump_ai.db（新增）
SQLite 数据库格式，专为大型游戏的 AI 查询优化。包含与 `dump_ai.json` 相同的数据，但提供：
- **索引查询** — 通过类型名、方法名、Assembly 快速定位
- **低 token 消耗** — 每次查询仅返回需要的行，而非整个文件
- **直接 SQL** — 适合 AI Agent 配合 `sqlite3` CLI 使用

表结构：

| 表名 | 说明 | 关键索引 |
|------|------|---------|
| `types` | 类型定义（名称、命名空间、Assembly、父类、可见性、修饰符、Token、大小） | `name`, `image` |
| `methods` | 方法定义（名称、签名、VA/RVA 地址、可见性、修饰符、Token、vtable slot） | `type_id`, `name` |
| `fields` | 字段定义（名称、类型、偏移） | `type_id` |
| `properties` | 属性定义（名称、getter/setter、Token） | `type_id` |
| `type_interfaces` | 类型-接口关联 | `type_id` |
| `nested_types` | 嵌套类型索引 | `type_id` |
| `images` | Assembly 映像信息 | - |
| `strings` | 字符串字面量 | - |
| `meta` | 元信息（含 `prompt` 字段） | - |

查询示例：
```bash
# 查找特定类的所有方法
sqlite3 dump_ai.db "SELECT name, signature, va FROM methods WHERE type_id = (SELECT id FROM types WHERE name='PlayerController')"

# 查找虚函数表
sqlite3 dump_ai.db "SELECT name, slot FROM methods WHERE is_virtual=1 AND type_id=42 ORDER BY slot"

# 统计各 Assembly 的类型数量
sqlite3 dump_ai.db "SELECT image_id, COUNT(*) FROM types GROUP BY image_id"
...
# Il2CppDumper ↔ Ghidra 集成 Skill

`docs/skill.md` 包含完整的 Il2CppDumper + Ghidra 逆向工作流说明，涵盖：

1. Il2CppDumper CLI dump 二进制 + metadata（含 `--ai-format` 选项说明）
2. 将 dump 结果导入 Ghidra 并运行脚本
3. 解析 `dump_ai.json` 或 `dump_ai.db`（SQLite）处理函数命名和结构体恢复
4. `sqlite3` CLI 查询示例 — 快速定位类型/方法/地址
5. 处理手动模式、内存 dump 等高级场景

Kilo 用户可在项目目录中执行 `skill il2cpp-ghidra` 加载此 skill 到当前会话。
